function Install-ServerPrerequisities {
	...
	Write-Host "Checking server for Windows Features - IIS Management Console..."
	$feature = Get-WindowsFeature Web-Mgmt-Console
	if (!$feature.Installed) {
		Write-Host "Windows Feature - IIS Management Console is not installed. Installing..."
		Install-WindowsFeature Web-Mgmt-Console
		Write-Verbose "Windows Feature - IIS Management Console has been installed."
	}
	else {
		Write-Host "Windows Feature - IIS Management Console has been detected. Skipping..."
	}
	...
	if (-not (Test-Path "${env:programfiles(x86)}\Microsoft SQL Server\130\DAC\bin\Microsoft.SqlServer.Types.dll"))
	{
		...
		Write-Host "CLR Types for SQL Server 2016 has been successfully installed."
	}
	else {
